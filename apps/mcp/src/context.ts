...
function preferenceLine(prefs: PreferenceDoc | null): string | null {
  ...
  const likes = prefs.likes.filter((s: string) => s.trim().length > 0);
  const dislikes = prefs.dislikes.filter((s: string) => s.trim().length > 0);
...
export function parseContextLimit(raw: number | undefined): number {
  if (raw === undefined || !Number.isFinite(raw)) return DEFAULT_CONTEXT_LIMIT;
  return clampContextLimit(raw);
}
...
export async function buildRecentContext(
  ...
  const enriched: RecipeWithFeedbackSummary[] = await Promise.all(
    recentRecipes.map(async (recipe: RecipeDoc) => {
